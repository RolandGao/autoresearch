...
def active_search_hparam_names():
    return tuple(name for name in SEARCH_HPARAM_ORDER if SEARCH_HPARAMS[name].search)
...
def find_best_hparam_point(
    ...
    def point_tta_val_acc(point):
        return results_by_point[point]["tta_val_acc"]

    def is_below_initial_main_lr_cutoff(point, max_tta_val_acc):
        tta_val_acc = point_tta_val_acc(point)
    ...
    def is_above_middle(point, middle_point):
    ...
    def search_direction(middle_point, index, first_point):
        ...
        while current_point is not None:
            ...
            if found_new_best_above_middle:
                ...
                if should_stop:
                    break
            next_point = next_point_along_direction(current_point, index, delta)
    ...
    def search_initial_main_lr(middle_point, index):
        name = search_names[index]

        def update_max_tta_val_acc(max_tta_val_acc, point):
            tta_val_acc = point_tta_val_acc(point)
            if not finite(tta_val_acc):
        ...
        def probe_side(probe_points, max_tta_val_acc, delta):
            previous_point = middle_point
            current_point = point_in_direction(middle_point, index, delta)
            for _ in range(INITIAL_MAIN_LR_SIDE_STEPS):
                if current_point is None:
                    break
                evaluate(
                    current_point,
                    cooldown_seed_point=previous_point,
                    cooldown_search_names=cooldown_search_hparam_names_for_step(
                        search_names[index]
                    ),
                )
                probe_points.append(current_point)
                if is_below_initial_main_lr_cutoff(current_point, max_tta_val_acc):
                    break
                max_tta_val_acc = update_max_tta_val_acc(
                    max_tta_val_acc, current_point
                )
                previous_point = current_point
                current_point = point_in_direction(current_point, index, delta)
            return max_tta_val_acc

        def is_above_final_cutoff(point, max_tta_val_acc):
            tta_val_acc = point_tta_val_acc(point)
        ...
        def lr_value(point):
            return hparam_from_state(name, point[index])

        probe_points = [middle_point]
        max_tta_val_acc = point_tta_val_acc(middle_point)
        max_tta_val_acc = probe_side(probe_points, max_tta_val_acc, -1)
        max_tta_val_acc = probe_side(probe_points, max_tta_val_acc, 1)
        candidate_points = [
            point
            for point in probe_points
            if is_above_final_cutoff(point, max_tta_val_acc)
        ]
        if not candidate_points:
            return middle_point
        return max(candidate_points, key=lr_value)

    def search_initial_main_momentum(middle_point, index):
        name = search_names[index]
        group = [
            point_with_state(middle_point, index, state)
            for state in neighbor_states(name, middle_point[index])
        ...
        group = lower_value_first(group, search_names, index)
        for neighbor_point in group:
            direction_best_point = search_direction(middle_point, index, neighbor_point)
            if direction_best_point != middle_point:
                if finalize_direction_best:
                    evaluate(direction_best_point)
                if better_point(
    ...
    def search_initial_main_once(middle_point):
        accepted_points = []
        for index in ordered_search_indexes(search_names):
            name = search_names[index]
            if SEARCH_HPARAMS[name].kind == "log_lr":
                next_point = search_initial_main_lr(middle_point, index)
            else:
                next_point = search_initial_main_momentum(middle_point, index)
    ...
    if initial_main_search:
        middle_point, accepted_points = search_initial_main_once(middle_point)
...
def search_hparam_segment(
    ...
    search_candidate_cooldown = (
        interval_info.get("search_candidate_cooldown", False)
        if interval_info is not None
        else False
    )

    def evaluate(point, cooldown_seed_point=None, cooldown_search_names=None):
        candidate_cooldown_search = (
            search_candidate_cooldown and cooldown_search_names is not None
        )
        needs_cooldown_state = (
            interval_info is not None
            and interval_info["use_cooldown"]
            and (fixed_cooldown_hparams is None or candidate_cooldown_search)
        )
        uses_fixed_cooldown = (
            interval_info is not None
            and interval_info["use_cooldown"]
        ...
        if cached_result is not None:
            ...
            cached_cooldown_search_set = (
                set(cached_cooldown_search_names)
                if cached_cooldown_search_names
                else set()
            )
            if (
                requested_cooldown_search_names is None
                and cached_cooldown_search_names in (None, ())
            ) or (
                requested_cooldown_search_names is not None
                and set(requested_cooldown_search_names).issubset(
                    cached_cooldown_search_set
                )
            ):
        ...
        if interval_info is not None:
            ...
            if requested_cooldown_search_names is not None:
                result["cooldown_search_names"] = requested_cooldown_search_names
            elif uses_fixed_cooldown:
                result["cooldown_search_names"] = ()
            else:
                result["cooldown_search_names"] = None
            ...
            if (
                ...
                if cooldown_seed_point is not None:
                    seed_result = results_by_point.get(cooldown_seed_point)
                    if seed_result is not None:
                        seed_cooldown_states = seed_result.get("cooldown_best_states")
                ...
                cooldown_best_states = {
                    name: nearest_hparam_state(name, cooldown_hparams[name])
                    for name in cooldown_search_hparam_names()
                }
                cooldown_result["tta_val_acc"] = cooldown_tta_val_acc
                cooldown_result["best_states"] = cooldown_best_states
                result["cooldown_result"] = cooldown_result
                result["cooldown_best_states"] = cooldown_best_states
...
def search_interval_hparams(
    ...
    def search_main_phase(
        ...
        initial_main_search = (
            fixed_cooldown_hparams is None and not search_candidate_cooldown
        )
        interval_info = dict(
            interval_index=interval_index,
