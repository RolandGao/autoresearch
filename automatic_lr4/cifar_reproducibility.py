...
def main(run, model, batch_size, muon_lr, seed):
...
if __name__ == "__main__":
    ...
    results = []
    for run in range(NUM_REPRO_RUNS):
        seed = TRAINING_SEED
        print(
            "cifar_reproducibility run=%d seed=%d batch_size=%d muon_lr=%.6g"
            % (run, seed, RUN_CONFIG["batch_size"], RUN_CONFIG["muon_lr"]),
            flush=True,
        )
        result = main(
            run,
    ...
    for result in results:
        print(
            "  run=%d seed=%d tta_val_acc=%.4f val_acc=%.4f train25_loss=%.4f"
            % (
                result["run"],
                result["seed"],
                result["tta_val_acc"],
    ...
    print("TTA val acc mean:   %.4f" % mean(tta_val_accs))
    print("TTA val acc std:    %.4f" % pstdev(tta_val_accs))
    print("TTA val acc min:    %.4f" % min(tta_val_accs))
    print("TTA val acc max:    %.4f" % max(tta_val_accs))
    print("TTA val acc range:  %.4f" % (max(tta_val_accs) - min(tta_val_accs)))
