...
LINE_SEARCH_LR_SUBGROUPS = ("LR_MULT",)
...
def update_optimizer_hyperparams(optimizer, step, subgroup_lrs):
    ...
    for group in optimizer.param_groups:
        ...
        group["lr"] = lr_mult_base_lr * subgroup_lrs[subgroup] * lrm
        if group["kind"] == "muon":
...
while step < MAX_STEPS:
    ...
    progress = min(step / max(1, MAX_STEPS - 1), 1.0)
    lrm = get_lr_multiplier(progress)
    required_future_count = required_future_train_batch_count(LINE_SEARCH_VAL_SET_NAME)
    ...
    print(
        f"\rstep {step:05d} ({pct_done:.5g}%) | loss: {debiased_smooth_loss:.5g} | lrm: {lrm:.5g} | lrs: {format_lr_summary(lr_by_subgroup)} | val_loss: {lr_search_result['val_loss']:.5g} | dt: {dt * 1000:.5g}ms | tok/sec: {tok_per_sec:,} | mfu: {mfu:.5g}% | epoch: {epoch} | remaining_steps: {remaining_steps}    ",
        end="",
        flush=True,
    )
...
print(f"depth:            {DEPTH}")
print(f"lr_mult:          {lr_by_subgroup['LR_MULT']:.5g}")
