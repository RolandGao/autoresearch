...
DEFAULT_LOG = Path(__file__).with_name("cifar_baseline2_eval2_n_search9.log")
DEFAULT_BASELINE_LOG = Path(__file__).with_name("cifar_baseline2_eval2_n_search2.log")
DEFAULT_OUTPUT_DIR = Path(__file__).with_name("cifar_baseline2_eval2_n_search9_plots")
...
def main():
    parser = argparse.ArgumentParser(
        description="Plot cifar_baseline2_eval2 n_search9 results with baseline."
    )
    ...
    plotter.plot_decay_search_tta(runs, args.output_dir / "decay_search_tta.png")
    ...
    print(
        ...
        f"and {len(baseline_runs)} baseline runs from {args.baseline_log}"
    )
    ...
    for run in plotter.sorted_runs(runs):
        ...
        if run.decay_final_selection is not None:
            decay = (
                f" decay={run.decay_final_selection['schedule']}"
                f" p={plotter.fmt(run.decay_final_selection['end_lr_p'])}"
            )
        print(
            f"run={run.index} bs={run.batch_size} kind={run.label} "
            f"train_loss={plotter.fmt(run.train_loss)} "
            f"val_acc={plotter.fmt(run.val_acc)} "
            f"tta={plotter.fmt(run.tta_val_acc)}{decay}"
        )
