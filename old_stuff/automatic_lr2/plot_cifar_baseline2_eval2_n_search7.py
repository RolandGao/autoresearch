...
DEFAULT_LOG = Path(__file__).with_name("cifar_baseline2_eval2_n_search7.log")
DEFAULT_BASELINE_LOG = Path(__file__).with_name("cifar_baseline2_eval2_n_search2.log")
DEFAULT_OUTPUT_DIR = Path(__file__).with_name("cifar_baseline2_eval2_n_search7_plots")
...
def main():
    parser = argparse.ArgumentParser(
        description="Plot cifar_baseline2_eval2 n_search7 results with baseline."
    )
    ...
    plotter.plot_lr_schedules_preclip_vs_applied(
        runs, args.output_dir / "preclip_vs_applied_lr_schedules.png"
    )
