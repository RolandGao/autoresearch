...
OVERFIT_MUON_MOMENTUMS = [0.0, 0.6]
OVERFIT_SGD_LR_MULT = 1.0
OVERFIT_BEST_LR_STRATEGY = "min_loss"
OVERFIT_BEST_LR_SCHEDULER = "constant"


def momentum_label(momentum):
    return ("%.8g" % momentum).replace("-", "m").replace(".", "p")


def overfit_run_config(batch_size, train_steps, muon_momentum):
    return dict(
        name=(
            f"min_loss_steps{train_steps}_bs{batch_size}_"
            f"momentum{momentum_label(muon_momentum)}"
        ),
        ...
        best_lr_strategy=OVERFIT_BEST_LR_STRATEGY,
        best_lr_scheduler=OVERFIT_BEST_LR_SCHEDULER,
    )
...
if __name__ == "__main__":
    ...
    def evaluate_config(batch_size, train_steps, muon_momentum):
        config = overfit_run_config(batch_size, train_steps, muon_momentum)
        print(
            "cifar_baseline2 run=%d train_steps=%d batch_size=%d "
            "muon_lr=%.6g muon_momentum=%.6g "
        ...
        print("Multiplier schedule:%s" % result["best_lr_scheduler"])
        if result["use_best_lr"]:
            print("Best lr strategy:   %s" % result["best_lr_strategy"])
            print("Best lr multiplier: %.6g" % 1.0)
    ...
    for batch_size in OVERFIT_BATCH_SIZES:
        for train_steps in OVERFIT_TRAIN_STEPS_LIST:
            for muon_momentum in OVERFIT_MUON_MOMENTUMS:
                result = evaluate_config(batch_size, train_steps, muon_momentum)
                ...
                print(
                    "min_loss_run complete train_steps=%d batch_size=%d "
                    "muon_momentum=%.6g multiplier=1 train_loss=%.4f "
    ...
    torch.save(
        dict(
            ...
            multiplier_scheduler=OVERFIT_BEST_LR_SCHEDULER,
            multiplier=1.0,
        ),
        log_path,
    )
