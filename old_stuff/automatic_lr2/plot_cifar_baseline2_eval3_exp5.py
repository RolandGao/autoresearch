...
DEFAULT_LOG = Path(__file__).with_name("cifar_baseline2_eval3_exp5.log")
DEFAULT_OUTPUT_DIR = Path(__file__).with_name("cifar_baseline2_eval3_exp5_plots")

RUN_RE = re.compile(
    r"^cifar_baseline2 run=(?P<run>\d+) batch_size=(?P<batch_size>\d+) "
    r"muon_lr=(?P<muon_lr>\S+) sgd_lr_mult=(?P<sgd_lr_mult>\S+) "
    r"end_muon_lr=(?P<end_muon_lr>\S+) name=(?P<name>\S+) "
...
NAME_POINT_RE = re.compile(
    r"_mu(?P<mu_exp>[pm]?\d+)_sgd(?P<sgd_exp>[pm]?\d+)_end(?P<end_exp>[pm]?\d+)_"
)
EVAL_RE = re.compile(
...
SEARCH_STEP_RE = re.compile(
    ...
    r"center=(?P<center>\([^)]+\)) center_tta=(?P<center_tta>\S+) "
    r"best=(?P<best>\([^)]+\)) best_tta=(?P<best_tta>\S+)"
)
SEARCH_COMPLETE_RE = re.compile(
    r"^joint_line_search complete batch_size=(?P<batch_size>\d+) "
    r"best_point=(?P<best_point>\([^)]+\)) muon_lr=(?P<muon_lr>\S+) "
    r"sgd_lr_mult=(?P<sgd_lr_mult>\S+) end_muon_lr=(?P<end_muon_lr>\S+) "
...
CACHE_HIT_RE = re.compile(
    ...
    r"point=(?P<point>\([^)]+\)) muon_lr=(?P<muon_lr>\S+) "
    r"sgd_lr_mult=(?P<sgd_lr_mult>\S+) end_muon_lr=(?P<end_muon_lr>\S+) "
...
def parse_point(text):
    ...
    if len(point) != 3:
        raise ValueError(f"Expected a 3D point, got {text}")
...
def point_from_name(name):
    ...
    return (
        ...
        parse_exp_label(match.group("end_exp")),
    )
        ...
        for line_number, line in enumerate(f, start=1):
            ...
            if match:
                current = Run(
                    ...
                    end_muon_lr=float(match.group("end_muon_lr")),
...
def metric_value(run, attr):
    value = getattr(run, attr)
    return value if finite(value) else np.nan


def plot_3d_search_surface(runs, search_steps, completions, output_path):
    sizes = batch_sizes(runs)
    fig = plt.figure(figsize=(7.2 * len(sizes), 6.4))
    axes = [fig.add_subplot(1, len(sizes), i + 1, projection="3d") for i in range(len(sizes))]

    for ax, batch_size in zip(axes, sizes):
        ...
        scatter = ax.scatter(xs, ys, zs, c=vals, cmap="viridis", s=90, edgecolor="black")
        for run in batch_runs:
            ax.text(run.point[0], run.point[1], run.point[2], f"r{run.index}\n{run.tta_val_acc:.4f}", fontsize=7)

        steps = sorted([step for step in search_steps if step.batch_size == batch_size], key=lambda step: step.move)
        if steps:
        ...
        ax.set_ylabel("sgd exponent")
        ax.set_zlabel("end muon exponent")
        fig.colorbar(scatter, ax=ax, shrink=0.65, label="TTA val acc")

    fig.suptitle("3D Joint Line Search Samples")
    fig.tight_layout()
    fig.savefig(output_path, dpi=180)
    plt.close(fig)


def plot_pairwise_projections(runs, completions, output_path):
    sizes = batch_sizes(runs)
    pairs = [
        (0, 1, "muon exponent", "sgd exponent"),
        (0, 2, "muon exponent", "end muon exponent"),
        (1, 2, "sgd exponent", "end muon exponent"),
    ]
    fig, axes = plt.subplots(
        len(pairs), len(sizes), figsize=(6.4 * len(sizes), 5.2 * len(pairs)), squeeze=False
    )
    for col, batch_size in enumerate(sizes):
        batch_runs = runs_for_batch(runs, batch_size)
        vals = np.array([run.tta_val_acc for run in batch_runs], dtype=float)
        for row, (x_idx, y_idx, x_label, y_label) in enumerate(pairs):
            ax = axes[row][col]
            scatter = ax.scatter(
                [run.point[x_idx] for run in batch_runs],
                [run.point[y_idx] for run in batch_runs],
                c=vals,
                cmap="viridis",
                s=120,
                edgecolor="black",
            ...
            for run in batch_runs:
                ax.annotate(f"r{run.index}\n{run.tta_val_acc:.4f}", (run.point[x_idx], run.point[y_idx]), ha="center", va="center", fontsize=7)
            completion = next((item for item in completions if item.batch_size == batch_size), None)
            if completion is not None:
                ax.scatter(
                    [completion.best_point[x_idx]],
                    [completion.best_point[y_idx]],
                    marker="*",
                    s=340,
                    color="gold",
                    edgecolor="black",
    ...
    fig.suptitle("Pairwise Projections of 3D Search")
    fig.tight_layout()
    fig.savefig(output_path, dpi=180)
    plt.close(fig)
...
def plot_search_progress(search_steps, completions, output_path):
    ...
    fig, axes = plt.subplots(1, len(sizes), figsize=(6.8 * len(sizes), 4.8), squeeze=False)
    for ax, batch_size in zip(axes.ravel(), sizes):
        steps = sorted([step for step in search_steps if step.batch_size == batch_size], key=lambda step: step.move)
        ax.plot([step.move for step in steps], [step.center_tta for step in steps], marker="o", label="center")
        ax.plot([step.move for step in steps], [step.best_tta for step in steps], marker="s", label="best neighbor/center")
        for step in steps:
            ax.annotate(f"{point_label(step.center)} -> {point_label(step.best)}", (step.move, step.best_tta), xytext=(4, 4), textcoords="offset points", fontsize=7)
        completion = next((row for row in completions if row.batch_size == batch_size), None)
        if completion is not None:
            ax.axhline(completion.tta_val_acc, color="tab:green", linestyle=":", label=f"selected {point_label(completion.best_point)}")
        ax.set_title(f"batch_size={batch_size}")
...
def plot_final_metrics(runs, output_path):
    ...
    labels = [f"r{run.index}\nbs{run.batch_size}\n{point_label(run.point)}" for run in ordered]
    ...
    metrics = [
        ...
        ("wall seconds", "wall_time_seconds"),
    ]
    fig, axes = plt.subplots(len(metrics), 1, figsize=(18, 15), sharex=True)
    for ax, (label, attr) in zip(axes, metrics):
        ax.bar(x, [metric_value(run, attr) for run in ordered])
        ax.set_ylabel(label)
...
def plot_eval_curves(runs, output_path):
    sizes = batch_sizes(runs)
    fig, axes = plt.subplots(1, len(sizes), figsize=(7.2 * len(sizes), 5.5), squeeze=False)
    for ax, batch_size in zip(axes.ravel(), sizes):
...
def write_summary(runs, search_steps, completions, cache_hits, output_path):
    ordered = sorted(runs, key=lambda run: (run.batch_size, run.index))
    lines = [
        "CIFAR Baseline Eval3 Exp5 3D Joint Line Search",
        "=" * 50,
        "",
    ...
    for completion in sorted(completions, key=lambda row: row.batch_size):
        lines.extend(
            [
                ...
                f"end_muon_lr: {fmt(completion.end_muon_lr)}",
    ...
    for batch_size in batch_sizes(runs):
        ...
        for rank, run in enumerate(ranked, start=1):
            lines.append(
                ...
                f"end_muon_lr={fmt(run.end_muon_lr)} tta_val_acc={fmt(run.tta_val_acc)}"
            )
        lines.append("")
...
def write_csv(runs, output_path):
    lines = [
        "run,batch_size,mu_exp,sgd_exp,end_exp,muon_lr,sgd_lr_mult,end_muon_lr,"
        "train_loss,val_loss,train_acc,val_acc,tta_val_acc,wall_time_seconds,cuda_time_seconds"
    ]
...
def main():
    parser = argparse.ArgumentParser(
        description="Plot cifar_baseline2_eval3 exp5 3D joint line-search results."
    )
    ...
    plot_3d_search_surface(runs, search_steps, completions, args.output_dir / "search_surface_3d.png")
    plot_pairwise_projections(runs, completions, args.output_dir / "search_pairwise_projections.png")
    ...
    for completion in sorted(completions, key=lambda row: row.batch_size):
        print(
            ...
            f"end_muon_lr={completion.end_muon_lr:.6g} tta={completion.tta_val_acc:.4f} "
