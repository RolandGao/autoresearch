...
def capture_training_state(model, optimizer):
    return (
        ...
        clone_state(optimizer.state_dict()),
        clone_grads(model),
        model.training,
    )
...
def restore_training_state(model, optimizer, state):
    model_state, optimizer_state, grads, was_training = state
    model.load_state_dict(model_state)
    optimizer.load_state_dict(clone_state(optimizer_state))
...
def update_lr_ema(ema_lr, line_search_lr):
    if ema_lr is None:
        return line_search_lr
    return LINE_SEARCH_EMA_BETA * ema_lr + (1 - LINE_SEARCH_EMA_BETA) * line_search_lr
...
def compute_line_search_loss(model, inputs, targets):
    with autocast_ctx:
        loss = model(inputs, targets)
    return loss


def line_search(model, state_model, optimizer, inputs, targets, base_matrix_lr):
    search_state = capture_training_state(state_model, optimizer)
    losses_by_exponent = {}
    losses_by_matrix_lr = {}

    def restore_search_state():
        restore_training_state(state_model, optimizer, search_state)

    def evaluate_candidate(exponent):
        if exponent in losses_by_exponent:
            return losses_by_exponent[exponent]
        matrix_lr = lr_from_exponent(base_matrix_lr, exponent)
        if matrix_lr in losses_by_matrix_lr:
            loss = losses_by_matrix_lr[matrix_lr]
            losses_by_exponent[exponent] = loss
            return loss
        restore_search_state()
        set_matrix_lr(optimizer, matrix_lr)
        optimizer.step()
        with torch.no_grad():
            loss = compute_line_search_loss(model, inputs, targets)
        loss = loss.item() if torch.isfinite(loss) else float("inf")
        losses_by_exponent[exponent] = loss
        losses_by_matrix_lr[matrix_lr] = loss
        return loss
    ...
    while True:
        ...
        best_exponent = min(
            candidates, key=lambda candidate: losses_by_exponent[candidate]
        )
        if best_exponent == center:
            restore_search_state()
            return (
                lr_from_exponent(base_matrix_lr, center),
                losses_by_exponent[center],
                losses_by_matrix_lr,
            )
        center = best_exponent


def evaluate_matrix_lr_losses(
    model,
    state_model,
    optimizer,
    inputs,
    ...
    for matrix_lr in matrix_lrs:
        ...
        with torch.no_grad():
            loss = compute_line_search_loss(model, inputs, targets)
        losses_by_matrix_lr[matrix_lr] = (
            loss.item() if torch.isfinite(loss) else float("inf")
        )

    restore_search_state()
    return losses_by_matrix_lr


def ground_truth_matrix_lr_sweep(ground_truth_matrix_lr):
    sweep = [0.0, ground_truth_matrix_lr]
    for i in range(1, 11):
        sweep.append(lr_from_exponent(ground_truth_matrix_lr, i))
        sweep.append(lr_from_exponent(ground_truth_matrix_lr, -i))
    return sweep
...
def second_line_search(
    ...
    search_state = capture_training_state(state_model, optimizer)
    set_matrix_lr(optimizer, first_matrix_lr)
    optimizer.step()

    state_model.zero_grad(set_to_none=True)
    first_step_loss = compute_line_search_loss(model, inputs, targets)
    first_step_loss.backward()
    second_matrix_lr, second_loss = line_search(
        model,
        state_model,
        optimizer,
        inputs,
    ...
    restore_training_state(state_model, optimizer, search_state)
    return second_matrix_lr, second_loss
...
step = 0
ema_line_search_matrix_lr = None
...
while True:
    ...
    ground_truth_matrix_lr = next(
        group["lr"] for group in optimizer.param_groups if group["kind"] == "muon"
    )
    line_search_base_matrix_lr = (
        ground_truth_matrix_lr
        if ema_line_search_matrix_lr is None
        else ema_line_search_matrix_lr
    )
    line_search_matrix_lr, line_search_loss, matrix_lr_train_losses = line_search(
        model,
        uncompiled_model,
        optimizer,
        line_search_x,
    ...
    matrix_lr_train_losses = evaluate_matrix_lr_losses(
        model,
        uncompiled_model,
        optimizer,
        line_search_x,
        line_search_y,
        ground_truth_matrix_lr_sweep(ground_truth_matrix_lr),
        matrix_lr_train_losses,
    )
    ema_line_search_matrix_lr = update_lr_ema(
        ema_line_search_matrix_lr, line_search_matrix_lr
    )
    print(
        "\nline_search step=%d "
