...
DEFAULT_LOG = Path(__file__).with_name("cifar_baseline2_eval3_exp2.log")
DEFAULT_OUTPUT_DIR = Path(__file__).with_name("cifar_baseline2_eval3_exp2_plots")


def write_exp2_summary(runs, search_steps, completions, cache_hits, output_path):
    ordered = sorted(runs, key=lambda run: (run.batch_size, run.index))
    lines = [
        "CIFAR Baseline Eval3 Exp2 Joint Line Search",
        "=" * 45,
        "",
...
def main():
    parser = argparse.ArgumentParser(
        description="Plot cifar_baseline2_eval3 exp2 joint line-search results."
    )
