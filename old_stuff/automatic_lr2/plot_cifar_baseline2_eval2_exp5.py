...
DEFAULT_LOG = Path(__file__).with_name("cifar_baseline2_eval2_exp5.log")
DEFAULT_OUTPUT_DIR = Path(__file__).with_name("cifar_baseline2_eval2_exp5_plots")
...
SCHEDULER_ORDER = ["constant", "linear", "last2_linear"]
...
def scheduler_sorted(runs):
    order = {name: index for index, name in enumerate(SCHEDULER_ORDER)}
    return sorted(
        runs,
        key=lambda run: (
            order.get(getattr(run, "best_lr_scheduler", ""), 99),
            run.index,
        ),
    )


def plot_scheduler_comparison(runs, output_path):
    metrics = [
        ("tta_val_acc", "TTA val acc", lambda run: run.tta_val_acc),
        ("val_acc", "val acc", lambda run: run.val_acc),
        ("train_loss", "train loss", base.final_train_loss),
        ("val_loss", "val loss", lambda run: getattr(run, "val_loss", None)),
        ("train_acc", "train acc", lambda run: getattr(run, "train_acc", None)),
        (
            "final_post_loss_mean",
        ...
        (
            "wall_time_seconds",
            "wall time seconds",
            lambda run: getattr(run, "wall_time_seconds", None),
        ),
        (
            "cuda_time_seconds",
            "CUDA time seconds",
            lambda run: getattr(run, "cuda_time_seconds", None),
        ),
    ]
    ...
    fig, axes = plt.subplots(
        len(metrics),
        len(grouped),
        figsize=(5.6 * len(grouped), 3.0 * len(metrics)),
        squeeze=False,
    )
    colors = {
        "constant": "tab:blue",
    ...
    for col, (batch_size, batch_runs) in enumerate(grouped.items()):
        batch_runs = scheduler_sorted(batch_runs)
        labels = [run_label(run) for run in batch_runs]
        x = np.arange(len(batch_runs))
        bar_colors = [colors.get(label, "tab:gray") for label in labels]
        for row, (_, ylabel, getter) in enumerate(metrics):
            ax = axes[row][col]
            values = [
                getter(run)
                if getter(run) is not None and math.isfinite(getter(run))
                else np.nan
    ...
    fig.suptitle("Peak-LR Scheduler Comparison")
    fig.tight_layout()
...
def plot_final_post_loss_distributions(runs, output_path):
    ...
    for ax, (batch_size, batch_runs) in zip(axes, runs_by_batch(runs).items()):
        batch_runs = scheduler_sorted(batch_runs)
        ...
        ax.grid(True, axis="y", alpha=0.25)
    axes[0].set_ylabel("final post-update batch losses")
...
def write_summary(runs, output_path):
    lines = [
        "run,name,batch_size,muon_lr,scheduler,train_loss,train25_loss,"
        "val_loss,train_acc,val_acc,tta_val_acc,"
        "final_post_loss_mean,final_post_loss_sum,wall_time_seconds,"
        "cuda_time_seconds,steps,final_applied_lr,final_searched_lr,"
        "final_best_lr_ema"
    ]
...
def write_important_info(runs, output_path):
    ...
    lines = [
        "CIFAR Baseline Exp5 Peak-LR Scheduler Results",
        "=" * 46,
        "",
        f"Runs: {len(runs)}",
...
def write_progress_summary(runs, output_path):
    lines = [
        "run,name,batch_size,scheduler,steps,total_steps,progress,best_lr_logs,"
        "eval_points,is_complete"
    ]
    for run in base.run_order(runs):
...
def main():
    parser = argparse.ArgumentParser(
        description="Plot cifar_baseline2_eval2 exp5 scheduler results."
    )
    parser.add_argument("--log", type=Path, default=DEFAULT_LOG)
    parser.add_argument("--output-dir", type=Path, default=DEFAULT_OUTPUT_DIR)
    args = parser.parse_args()

    runs = parse_exp5_log(args.log)
    ...
    plot_scheduler_comparison(runs, args.output_dir / "scheduler_comparison.png")
    ...
    for run in base.run_order(runs):
        ...
        print(
            f"run={run.index} name={run.name} scheduler={run_label(run)} "
