...
def _double_center(G):
    return G - G.mean(dim=1, keepdim=True) - G.mean(dim=0, keepdim=True) + G.mean()


def _classic_ns_scaled(G, steps=4, center=False, row_balance_power=None):
    X = G
    if row_balance_power is not None:
        X = _row_balanced_gradient(X, power=row_balance_power)
    if center:
        X = _double_center(X)
    update = zeropower_via_newtonschulz5(X, steps=steps, eps=_UPDATE_EPS)
    return _match_frobenius(update.float(), _target_frobenius(G)).to(G.dtype)


def _aurora_soft_scaled(G, row_balance_power=0.5):
    return _track3_soft_scaled(_row_balanced_gradient(G, power=row_balance_power))


def _blend_to_target_norm(G, first, second, second_weight):
    blended = first.float() * (1 - second_weight) + second.float() * second_weight
    return _match_frobenius(blended, _target_frobenius(G)).to(G.dtype)


def _classic_track3_blend(G, classic_steps=4, track3_weight=0.5, row_balance_power=None):
    X = G if row_balance_power is None else _row_balanced_gradient(G, row_balance_power)
    classic = _classic_ns_scaled(X, steps=classic_steps)
    track3 = _track3_scaled_polar(X)
    return _blend_to_target_norm(G, classic, track3, track3_weight)


def _classic_soft_blend(G, classic_steps=4, soft_weight=0.5, row_balance_power=None):
    X = G if row_balance_power is None else _row_balanced_gradient(G, row_balance_power)
    classic = _classic_ns_scaled(X, steps=classic_steps)
    soft = _track3_soft_scaled(X)
    return _blend_to_target_norm(G, classic, soft, soft_weight)


def _classic_qr_blend(G, classic_steps=4, qr_weight=0.35, row_balance_power=None):
    X = G if row_balance_power is None else _row_balanced_gradient(G, row_balance_power)
    classic = _classic_ns_scaled(X, steps=classic_steps)
    qr = qr_row_orthogonal(X)
    return _blend_to_target_norm(G, classic, qr, qr_weight)


def _contra_soft_blend_tuned(
    G,
    step,
    total_steps,
    contra_coeff=-0.2,
    ...
    progress = _schedule_fraction(step, total_steps)
    contra = contra_coeff * (1.0 - _linear_ramp(progress, 0.0, contra_end))
    update = polar + contra * normalized_grad
    update = update * (reference_norm / _gram_frobenius_norm_estimate(update)).to(
        update.dtype
    )

    soft = soft_track3(G)
    soft = soft * (reference_norm / _gram_frobenius_norm_estimate(soft)).to(soft.dtype)
    soft_blend = min(soft_ceil, _linear_ramp(progress, soft_start, soft_end))
    update = update + (soft - update) * soft_blend
    update = update * (reference_norm / _gram_frobenius_norm_estimate(update)).to(
        update.dtype
    )
...
@context_update
def ns4_track3_blend25(G, **kwargs):
    return _classic_track3_blend(G, classic_steps=4, track3_weight=0.25)


@context_update
def ns4_track3_blend50(G, **kwargs):
    return _classic_track3_blend(G, classic_steps=4, track3_weight=0.50)


@context_update
def ns4_track3_blend75(G, **kwargs):
    return _classic_track3_blend(G, classic_steps=4, track3_weight=0.75)


@context_update
def ns4_aurora_half_blend50(G, **kwargs):
    return _classic_track3_blend(G, classic_steps=4, track3_weight=0.50, row_balance_power=0.5)


@context_update
def ns4_aurora_quarter(G, **kwargs):
    return _classic_ns_scaled(G, steps=4, row_balance_power=0.25)


@context_update
def ns4_aurora_threequarter(G, **kwargs):
    return _classic_ns_scaled(G, steps=4, row_balance_power=0.75)


@context_update
def ns4_centered_aurora_half(G, **kwargs):
    return _classic_ns_scaled(G, steps=4, center=True, row_balance_power=0.5)


@context_update
def ns4_normon_beta90(G, state=None, **kwargs):
    update = _classic_ns_scaled(G, steps=4)
    return _normon_postcondition(update, state, key="ns4_normon_beta90", beta2=0.90)


@context_update
def ns4_aurora_half_normon(G, state=None, **kwargs):
    update = _classic_ns_scaled(G, steps=4, row_balance_power=0.5)
    return _normon_postcondition(update, state, key="ns4_aurora_half_normon")


@context_update
def ns5_muon(G, **kwargs):
    return _classic_ns_scaled(G, steps=5)


@context_update
def ns5_aurora_half(G, **kwargs):
    return _classic_ns_scaled(G, steps=5, row_balance_power=0.5)


@context_update
def ns6_muon(G, **kwargs):
    return _classic_ns_scaled(G, steps=6)


@context_update
def ns3_aurora_half(G, **kwargs):
    return _classic_ns_scaled(G, steps=3, row_balance_power=0.5)


@context_update
def soft_aurora_quarter(G, **kwargs):
    return _aurora_soft_scaled(G, row_balance_power=0.25)


@context_update
def soft_aurora_half(G, **kwargs):
    return _aurora_soft_scaled(G, row_balance_power=0.5)


@context_update
def soft_aurora_threequarter(G, **kwargs):
    return _aurora_soft_scaled(G, row_balance_power=0.75)


@context_update
def soft_aurora_half_normon(G, state=None, **kwargs):
    update = _aurora_soft_scaled(G, row_balance_power=0.5)
    return _normon_postcondition(update, state, key="soft_aurora_half_normon")


@context_update
def contra_soft_aurora_half(G, step=0, total_steps=None, **kwargs):
    X = _row_balanced_gradient(G, power=0.5)
    return _contra_soft_blend_tuned(X, step, total_steps)


@context_update
def contra_fast_soft_aurora_half(G, step=0, total_steps=None, **kwargs):
    X = _row_balanced_gradient(G, power=0.5)
    return _contra_soft_blend_tuned(
        X, step, total_steps, contra_end=0.45, soft_start=0.60, soft_end=0.92
    )


@context_update
def ns4_qr_aurora_half_blend(G, **kwargs):
    return _classic_qr_blend(G, classic_steps=4, qr_weight=0.35, row_balance_power=0.5)


def _target_matrix_norm(G, update):
    return _match_frobenius(update.float(), _target_frobenius(G)).to(G.dtype)


def _elementwise_adam_precondition(G, state, key, beta2=0.95, eps=1e-8):
    Gf = G.float()
    if key not in state or state[key].shape != G.shape:
        state[key] = torch.zeros_like(Gf)
    state[key].lerp_(Gf.square(), 1 - beta2)
    return Gf / state[key].sqrt().clamp_min(eps)


def _stateful_adafactor_precondition(
    G, state, prefix, beta2=0.95, eps=1e-8, clip_rms=None
):
    Gf = G.float()
    row_key, col_key = f"{prefix}_row_ms", f"{prefix}_col_ms"
    row_ms = Gf.square().mean(dim=1, keepdim=True)
    col_ms = Gf.square().mean(dim=0, keepdim=True)
    if row_key not in state or state[row_key].shape != row_ms.shape:
        state[row_key] = torch.zeros_like(row_ms)
        state[col_key] = torch.zeros_like(col_ms)
    state[row_key].lerp_(row_ms, 1 - beta2)
    state[col_key].lerp_(col_ms, 1 - beta2)
    global_ms = state[row_key].mean().clamp_min(eps)
    rms = (state[row_key] * state[col_key] / global_ms).sqrt().clamp_min(eps)
    update = Gf / rms
    if clip_rms is not None:
        update_rms = update.square().mean().sqrt().clamp_min(eps)
        update = update / (update_rms / clip_rms).clamp_min(1.0)
    return update


def _ema_sign_update(G, state, key, beta=0.9):
    Gf = G.float()
    if key not in state or state[key].shape != G.shape:
        state[key] = torch.zeros_like(Gf)
    state[key].lerp_(Gf, 1 - beta)
    return state[key].sign()


def _inverse_power_from_cov(cov, power, eps=1e-8):
    evals, q = torch.linalg.eigh(
        cov + eps * torch.eye(cov.size(0), dtype=cov.dtype, device=cov.device)
    )
    return q @ torch.diag(evals.clamp_min(eps).pow(-power)) @ q.T


def _right_cov_precondition(G, state, prefix, beta2=0.95, power=0.5):
    Gf = G.float()
    key = f"{prefix}_right_cov"
    cov = Gf.T @ Gf / max(1, Gf.size(0))
    if key not in state or state[key].shape != cov.shape:
        state[key] = torch.zeros_like(cov)
    state[key].lerp_(cov, 1 - beta2)
    right = _inverse_power_from_cov(state[key], power)
    return Gf @ right


def _left_cov_precondition(G, state, prefix, beta2=0.95, power=0.5):
    Gf = G.float()
    key = f"{prefix}_left_cov"
    cov = Gf @ Gf.T / max(1, Gf.size(1))
    if key not in state or state[key].shape != cov.shape:
        state[key] = torch.zeros_like(cov)
    state[key].lerp_(cov, 1 - beta2)
    left = _inverse_power_from_cov(state[key], power)
    return left @ Gf


def _bilateral_cov_precondition(G, state, prefix, beta2=0.95, power=0.3):
    left = _left_cov_precondition(G, state, f"{prefix}_left", beta2=beta2, power=power)
    right = _right_cov_precondition(G, state, f"{prefix}_right", beta2=beta2, power=power)
    return 0.5 * (left + right)


def _muloco_extrapolate(update, state, key="muloco", momentum=0.5, blend=0.7):
    if f"{key}_velocity" not in state or state[f"{key}_velocity"].shape != update.shape:
        state[f"{key}_velocity"] = torch.zeros_like(update.float())
    velocity = state[f"{key}_velocity"]
    velocity.mul_(momentum).add_(update.float())
    extrapolated = update.float() + blend * velocity
    return _match_gram_frobenius_norm(extrapolated, update).to(update.dtype)


def _late_rre_extrapolate(
    update, state, step, total_steps, key="rre", start=0.75, damping=0.875, max_rel=0.05
):
    update_f = update.float()
    prev_key = f"{key}_prev"
    if prev_key not in state or state[prev_key].shape != update.shape:
        state[prev_key] = update_f.detach().clone()
        return update
    progress = _schedule_fraction(step, total_steps)
    if progress < start:
        state[prev_key].copy_(update_f)
        return update
    delta = update_f - state[prev_key]
    max_norm = update_f.norm().clamp_min(_UPDATE_EPS) * max_rel
    delta = delta * (max_norm / delta.norm().clamp_min(_UPDATE_EPS)).clamp(max=1.0)
    state[prev_key].copy_(update_f)
    extrapolated = update_f + damping * delta
    return _match_gram_frobenius_norm(extrapolated, update).to(update.dtype)


@context_update
def adamw_precond_norm(G, state=None, **kwargs):
    update = _elementwise_adam_precondition(G, state, key="adamw_v")
    return _target_matrix_norm(G, update)


@context_update
def adamh_radial_precond(G, state=None, param=None, **kwargs):
    update = _target_matrix_norm(
        G, _elementwise_adam_precondition(G, state, key="adamh_v")
    )
    return _radial_brake(update, param, brake=1.0)


@context_update
def adafactor_norm(G, state=None, **kwargs):
    return _target_matrix_norm(
        G, _stateful_adafactor_precondition(G, state, prefix="adafactor")
    )


@context_update
def adafactor_clipped_norm(G, state=None, **kwargs):
    return _target_matrix_norm(
        G,
        _stateful_adafactor_precondition(
            G, state, prefix="adafactor_clip", clip_rms=1.0
        ),
    )


@context_update
def signum_norm(G, **kwargs):
    return _target_matrix_norm(G, G.sign())


@context_update
def lion_sign_norm(G, state=None, **kwargs):
    return _target_matrix_norm(G, _ema_sign_update(G, state, key="lion_avg"))


@context_update
def signum_rms_hybrid(G, state=None, **kwargs):
    adam = _elementwise_adam_precondition(G, state, key="sign_rms_v")
    update = 0.5 * adam + 0.5 * G.sign()
    return _target_matrix_norm(G, update)


@context_update
def raw_shampoo_norm(G, state=None, **kwargs):
    update = _shampoo_precondition(G, state, prefix="raw_shampoo")
    return _target_matrix_norm(G, update)


@context_update
def spectral_descent_raw(G, state=None, **kwargs):
    update = _shampoo_precondition(
        G, state, prefix="spectral_descent", beta2=0.0, power=0.25
    )
    return _target_matrix_norm(G, update)


@context_update
def raw_soap_norm(G, state=None, **kwargs):
    update = _soap_precondition(G, state, prefix="raw_soap", blend=1.0)
    return _target_matrix_norm(G, update)


@context_update
def raw_kl_soap_norm(G, state=None, **kwargs):
    update = _soap_precondition(
        G, state, prefix="raw_kl_soap", beta2=0.95, denom_power=0.35, denom_floor_ratio=0.2
    )
    return _target_matrix_norm(G, update)


@context_update
def sinksoap_raw_norm(G, state=None, **kwargs):
    update = _sinkhorn_precondition(
        _soap_precondition(G, state, prefix="sinksoap_raw", blend=0.8)
    )
    return _target_matrix_norm(G, update)


@context_update
def psgd_kron_whiten_norm(G, state=None, **kwargs):
    update = _bilateral_cov_precondition(G, state, prefix="psgd", power=0.5)
    return _target_matrix_norm(G, update)


@context_update
def pmuon_bilateral_power_raw(G, state=None, **kwargs):
    update = _bilateral_cov_precondition(G, state, prefix="pmuon_raw", power=0.3)
    return _target_matrix_norm(G, update)


@context_update
def right_newton_cov_raw(G, state=None, **kwargs):
    update = _right_cov_precondition(G, state, prefix="right_newton", power=0.5)
    return _target_matrix_norm(G, update)


@context_update
def left_newton_cov_raw(G, state=None, **kwargs):
    update = _left_cov_precondition(G, state, prefix="left_newton", power=0.5)
    return _target_matrix_norm(G, update)


@context_update
def adam_soda_anchor(G, state=None, step=0, total_steps=None, param=None, lr=None):
    update = adamw_precond_norm(G, state=state)
    return _soda_anchor_term(update, state, param, lr, step, total_steps, strength=0.02)


@context_update
def adam_radial_brake(G, state=None, param=None, **kwargs):
    update = adamw_precond_norm(G, state=state)
    return _radial_brake(update, param, brake=0.75)


@context_update
def muloco_update_extrap(G, state=None, **kwargs):
    update = _target_matrix_norm(G, G.float())
    return _muloco_extrapolate(update, state, key="muloco_raw")


@context_update
def late_rre_update_extrap(G, state=None, step=0, total_steps=None, **kwargs):
    update = _target_matrix_norm(G, G.float())
    return _late_rre_extrapolate(update, state, step, total_steps, key="late_rre")
...
# New variants based on the strongest agentic_exp1 and agentic2_exp1 results.
UPDATE_CONFIGS = [
    update_config("ns4_track3_blend25", ns4_track3_blend25, muon_lr=0.22),
    update_config("ns4_track3_blend50", ns4_track3_blend50, muon_lr=0.22),
    update_config("ns4_track3_blend75", ns4_track3_blend75, muon_lr=0.18),
    update_config("ns4_aurora_half_blend50", ns4_aurora_half_blend50, muon_lr=0.18),
    update_config("ns4_aurora_quarter", ns4_aurora_quarter, muon_lr=0.22),
    update_config("ns4_aurora_threequarter", ns4_aurora_threequarter, muon_lr=0.18),
    update_config("ns4_centered_aurora_half", ns4_centered_aurora_half, muon_lr=0.18),
    update_config("ns4_normon_beta90", ns4_normon_beta90, muon_lr=0.22),
    update_config("ns4_aurora_half_normon", ns4_aurora_half_normon, muon_lr=0.18),
    update_config("ns5_muon", ns5_muon, muon_lr=0.22),
    update_config("ns5_aurora_half", ns5_aurora_half, muon_lr=0.18),
    update_config("ns6_muon", ns6_muon, muon_lr=0.22),
    update_config("ns3_aurora_half", ns3_aurora_half, muon_lr=0.22),
    update_config("soft_aurora_quarter", soft_aurora_quarter, muon_lr=0.22),
    update_config("soft_aurora_half", soft_aurora_half, muon_lr=0.18),
    update_config("soft_aurora_threequarter", soft_aurora_threequarter, muon_lr=0.18),
    update_config("soft_aurora_half_normon", soft_aurora_half_normon, muon_lr=0.18),
    update_config("contra_soft_aurora_half", contra_soft_aurora_half, muon_lr=0.22),
    update_config("contra_fast_soft_aurora_half", contra_fast_soft_aurora_half, muon_lr=0.22),
    update_config("ns4_qr_aurora_half_blend", ns4_qr_aurora_half_blend, muon_lr=0.18),
    update_config("adamw_precond_norm", adamw_precond_norm, muon_lr=0.061),
    update_config("adamh_radial_precond", adamh_radial_precond, muon_lr=0.061),
    update_config("adafactor_norm", adafactor_norm, muon_lr=0.061),
    update_config("adafactor_clipped_norm", adafactor_clipped_norm, muon_lr=0.061),
    update_config("signum_norm", signum_norm, muon_lr=0.025),
    update_config("lion_sign_norm", lion_sign_norm, muon_lr=0.025),
    update_config("signum_rms_hybrid", signum_rms_hybrid, muon_lr=0.049),
    update_config("raw_shampoo_norm", raw_shampoo_norm, muon_lr=0.061),
    update_config("spectral_descent_raw", spectral_descent_raw, muon_lr=0.061),
    update_config("raw_soap_norm", raw_soap_norm, muon_lr=0.061),
    update_config("raw_kl_soap_norm", raw_kl_soap_norm, muon_lr=0.061),
    update_config("sinksoap_raw_norm", sinksoap_raw_norm, muon_lr=0.061),
    update_config("psgd_kron_whiten_norm", psgd_kron_whiten_norm, muon_lr=0.061),
    update_config("pmuon_bilateral_power_raw", pmuon_bilateral_power_raw, muon_lr=0.061),
    update_config("right_newton_cov_raw", right_newton_cov_raw, muon_lr=0.061),
    update_config("left_newton_cov_raw", left_newton_cov_raw, muon_lr=0.061),
    update_config("adam_soda_anchor", adam_soda_anchor, muon_lr=0.061),
    update_config("adam_radial_brake", adam_radial_brake, muon_lr=0.061),
    update_config("muloco_update_extrap", muloco_update_extrap, muon_lr=0.061),
    update_config("late_rre_update_extrap", late_rre_update_extrap, muon_lr=0.061),
]
assert len(UPDATE_CONFIGS) == 40
RUN_CONFIGS = [
    dict(
        **batch_config,
        **update_config,
