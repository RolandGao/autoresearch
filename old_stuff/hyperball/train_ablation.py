...
class Block(nn.Module):
    ...
    def forward(self, x, ve, cos_sin, window_size, activation_norms=None, prefix=""):
        ...
        x = x + self.mlp(rms_norm(x), activation_norms, prefix)
        return x
...
class GPT(nn.Module):
    ...
    @torch.no_grad()
    def init_weights(self, norm_constrained_weight_kind=None):
        weight_norm = {
        ...
        if norm_constrained_weight_kind not in NORM_CONSTRAINED_WEIGHT_KINDS:
            raise ValueError(
                f"unknown norm-constrained weight kind: {norm_constrained_weight_kind}"
            )

        init_scheme = []

        def init_norm_constrained(w, key):
            torch.nn.init.normal_(w, mean=0.0, std=1.0)
            init_scheme.append((w, actual_frobenius_norm(w, weight_norm[key])))

        # Embedding and unembedding
        if norm_constrained_weight_kind == "wte":
            init_norm_constrained(self.transformer.wte.weight, "wte")
        else:
            torch.nn.init.normal_(self.transformer.wte.weight, mean=0.0, std=1.0)
        if norm_constrained_weight_kind == "lm_head":
            init_norm_constrained(self.lm_head.weight, "lm_head")
        else:
        ...
        for layer_idx, block in enumerate(self.transformer.h):
            ...
            matrix_weights = (
                ("q", block.attn.c_q.weight, f"{prefix}.q", "uniform"),
                ("k", block.attn.c_k.weight, f"{prefix}.k", "uniform"),
                ("v", block.attn.c_v.weight, f"{prefix}.v", "uniform"),
                (
                    "attn.c_proj",
                ...
                ("mlp.c_fc", block.mlp.c_fc.weight, f"{prefix}.mlp.c_fc", "uniform"),
                (
                    "mlp.c_proj",
            ...
            for weight_kind, w, key, fallback_init in matrix_weights:
                if norm_constrained_weight_kind == weight_kind:
                    init_norm_constrained(w, key)
                elif fallback_init == "uniform":
                    torch.nn.init.uniform_(w, -s, s)
                elif fallback_init == "zero":
                    torch.nn.init.zeros_(w)
                else:
                    raise NotImplementedError(fallback_init)
            if block.attn.ve_gate is not None:
                w = block.attn.ve_gate.weight
                key = f"{prefix}.attn.ve_gate"
                if norm_constrained_weight_kind == "attn.ve_gate":
                    init_norm_constrained(w, key)
                else:
                    torch.nn.init.zeros_(w)

        for layer_idx, ve in self.value_embeds.items():
            if norm_constrained_weight_kind == "ve":
                init_norm_constrained(ve.weight, f"h.{layer_idx}.ve")
            else:
    ...
    def setup_optimizer(
        ...
        norm_matrix_param_specs = []
        legacy_matrix_param_specs = []
        for block in self.transformer.h:
            ...
            for weight_kind, param in block_matrix_specs:
                target = (
                    norm_matrix_param_specs
                    if weight_kind == norm_constrained_weight_kind
                    else legacy_matrix_param_specs
                )
                target.append((weight_kind, param))
            if block.attn.ve_gate is not None:
                spec = ("attn.ve_gate", block.attn.ve_gate.weight)
                if norm_constrained_weight_kind == "attn.ve_gate":
                    norm_matrix_param_specs.append(spec)
                else:
                    legacy_matrix_param_specs.append(spec)
        norm_matrix_params = [p for _, p in norm_matrix_param_specs]
        legacy_matrix_params = [p for _, p in legacy_matrix_param_specs]
        ...
        assert len(list(self.parameters())) == (
            len(norm_matrix_params)
            + len(legacy_matrix_params)
        ...
        print(f"Scaling fallback AdamW LRs by 1/sqrt({model_dim}/768) = {dmodel_lr_scale:.6f}")

        param_groups = []

        def add_embedding_group(weight_kind, params, fallback_lr, fallback_wd):
            if norm_constrained_weight_kind == weight_kind:
                param_groups.append(
                    dict(
                        kind="adamh",
                ...
                param_groups.append(
                    dict(
                        ...
                        weight_kind=weight_kind,
                        params=params,
                        lr=fallback_lr * dmodel_lr_scale,
                        betas=adam_betas,
                        eps=1e-10,
                        weight_decay=fallback_wd,
                    )
                )

        add_embedding_group("lm_head", lm_head_params, UNEMBEDDING_LR, LM_HEAD_WD)
        add_embedding_group("wte", embedding_params, EMBEDDING_LR, EMBEDDING_WD)
        add_embedding_group("ve", value_embeds_params, EMBEDDING_LR, VALUE_EMBEDDING_WD)
        param_groups.extend(
            [
                dict(
                    kind="adamw",
                    weight_kind="resid_lambdas",
                    params=resid_params,
                    lr=scalar_lr * 0.01,
                    betas=adam_betas,
                    eps=1e-10,
        ...
        legacy_matrix_group_keys = sorted(
            {tuple(param.shape) for _, param in legacy_matrix_param_specs}
        )
        for shape in legacy_matrix_group_keys:
            group_params = [
                param
                for _, param in legacy_matrix_param_specs
                if tuple(param.shape) == shape
            ]
            param_groups.append(
                dict(
                    kind="muon_legacy",
                    weight_kind="fallback_matrix",
                    params=group_params,
                    lr=MATRIX_LR,
                    momentum=0.95,
                    ns_steps=5,
        ...
        norm_matrix_group_keys = sorted(
            {
                (weight_kind, tuple(param.shape))
                for weight_kind, param in norm_matrix_param_specs
            }
        )
        for weight_kind, shape in norm_matrix_group_keys:
            group_params = [
                param
                for param_kind, param in norm_matrix_param_specs
...
LR_MULT = 1.5
EMBEDDING_LR = 0.6 * LR_MULT  # fallback AdamW LR for token/value embeddings
UNEMBEDDING_LR = 0.004 * LR_MULT  # fallback AdamW LR for lm_head
MATRIX_LR = 0.04 * LR_MULT  # fallback Muon LR for transformer matrices
...
LM_HEAD_WD = 0.01  # fallback AdamW weight decay for lm_head
EMBEDDING_WD = 0.001  # fallback AdamW weight decay for token embeddings
VALUE_EMBEDDING_WD = 0.003  # fallback AdamW weight decay for value embeddings
NORM_CONSTRAINED_WEIGHT_KINDS = tuple(FITTED_EFFECTIVE_LR_BY_WEIGHT_KIND)
N_RUNS = len(NORM_CONSTRAINED_WEIGHT_KINDS)
RUN_WEIGHT_KINDS = NORM_CONSTRAINED_WEIGHT_KINDS[:N_RUNS]
...
print(f"Norm-constrained run order: {', '.join(RUN_WEIGHT_KINDS)}")
...
def run_training(norm_constrained_weight_kind, run_idx, num_runs):
    print(
        f"\n=== RUN {run_idx + 1}/{num_runs}: "
        f"norm_constrained_weight_kind={norm_constrained_weight_kind} ===",
        flush=True,
    )
    ...
    model.init_weights(norm_constrained_weight_kind)
    ...
    optimizer = model.setup_optimizer(
        norm_constrained_weight_kind=norm_constrained_weight_kind,
        matrix_lrs=FITTED_EFFECTIVE_LR_BY_WEIGHT_KIND,
    ...
    while True:
        ...
        if should_log_norms:
            ...
            norm_log_record["norm_constrained_weight_kind"] = (
                norm_constrained_weight_kind
            )
        ...
        print(
            f"\rrun {run_idx + 1}/{num_runs} {norm_constrained_weight_kind} | step {step:05d} ({pct_done:.1f}%) | loss: {debiased_smooth_loss:.6f} | lrm_non_scalar: {lrm_non_scalar:.2f} | lrm_old_train: {lrm_old_train:.2f} | dt: {dt * 1000:.0f}ms | tok/sec: {tok_per_sec:,} | mfu: {mfu:.1f}% | epoch: {epoch} | remaining_steps: {remaining_steps}    ",
            end="",
            flush=True,
        )
...
run_summaries = []
for run_idx, weight_kind in enumerate(RUN_WEIGHT_KINDS):
    run_summaries.append(run_training(weight_kind, run_idx, len(RUN_WEIGHT_KINDS)))
