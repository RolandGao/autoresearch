...
def line_search(model, state_model, optimizer, inputs, targets, base_matrix_lr):
    ...
    while True:
        ...
        best_exponent = min(candidates, key=lambda candidate: losses[candidate])
        if best_exponent == center:
            restore_search_state()
            return lr_from_exponent(base_matrix_lr, center), losses[center]
...
while True:
    ...
    line_search_matrix_lr, line_search_loss = line_search(
        model,
        uncompiled_model,
        optimizer,
        line_search_x,
    ...
    second_line_search_matrix_lr, second_line_search_loss = second_line_search(
        model,
        uncompiled_model,
        optimizer,
        line_search_x,
    ...
    print(
        ...
        "train_loss=%.6f line_search_loss=%.6f "
        "second_line_search_matrix_lr=%.8g second_line_search_loss=%.6f"
        % (
            step,
            line_search_matrix_lr,
            ground_truth_matrix_lr,
            train_loss.item(),
            line_search_loss,
            second_line_search_matrix_lr,
            second_line_search_loss,
        ),
        flush=True,
    )
    if should_log_norms:
