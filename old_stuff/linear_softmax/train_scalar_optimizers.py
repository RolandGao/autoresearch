...
ADAM_BETA2 = 0.95
ADAM_EPS = 1e-10
ADAM2_NESTEROV_OPTIONS = (False, True)
ADAM2_DISABLE_BIAS1_OPTIONS = (False, True)
ADAM2_ADAPTIVE_NORM_OPTIONS = (False, True)
ADAM_LR_GRID = tuple(
    2.0**exponent for exponent in range(-7, 11)
)
LR_DECAY = 4.0
LR_POWER = 1.0
...
def option_specs_for_optimizer(optimizer: str) -> tuple[dict[str, bool], ...]:
    ...
    if optimizer == "Adam2":
        return tuple(
            ...
            for nesterov in ADAM2_NESTEROV_OPTIONS
            for disable_bias1 in ADAM2_DISABLE_BIAS1_OPTIONS
            for adaptive_norm in ADAM2_ADAPTIVE_NORM_OPTIONS
        )
...
def scalar_batch_sse_loss_and_grad_compiled(
    ...
    return scalar_batch_sse_loss_and_grad(
                    ...
                    for option_spec in option_specs:
                        ...
                        for lr in ADAM_LR_GRID:
                            for beta1 in BETA1_MOMENTUM_VALUES:
                                ...
                                if optimizer not in ("AdamW", "Adam2"):
                                    raise ValueError(f"unknown optimizer: {optimizer}")
    ...
    for start in range(0, fixed_logits.size(0), config.eval_batch_size):
        ...
        if config.compile:
            output_probs = softmax_probs_compiled(
                log_scalars, fixed_logits[start:end]
            )
        else:
...
def adam2_step(
    ...
    exp_avg.lerp_(grad, 1 - beta1)
    if adaptive_norm:
        exp_avg_sq.lerp_(grad.abs(), 1 - beta2)
    else:
        exp_avg_sq.lerp_(grad.square(), 1 - beta2)
    if bias1 is None:
        bias1 = 1 - beta1**step_count
    ...
    numerator = exp_avg
    if nesterov:
        numerator = exp_avg * beta1 + grad * (1 - beta1)
    if not disable_bias1:
        numerator = numerator / bias1

    denom = exp_avg_sq / bias2
    if not adaptive_norm:
        denom = denom.sqrt()
    log_scalars.addcdiv_(numerator, denom.add(eps), value=-lr)
...
def adam2_graph_step(
    ...
    exp_avg.copy_(exp_avg * beta1 + grad * (1 - beta1))
    if adaptive_norm:
        exp_avg_sq.copy_(exp_avg_sq * beta2 + grad.abs() * (1 - beta2))
    else:
        exp_avg_sq.copy_(exp_avg_sq * beta2 + grad.square() * (1 - beta2))
...
def replay_cuda_graph_training(
    ...
    disable_bias1 = bool(hparams.get("disable_bias1", False))
    adaptive_norm = bool(hparams.get("adaptive_norm", False))

    def replayed_step() -> torch.Tensor:
        if optimizer == "Adam2":
            return adam2_graph_step(
                log_scalars,
                exp_avg,
                exp_avg_sq,
                static_logits,
    ...
    for step in range(prepared.steps):
        ...
        graph.replay()
    torch.cuda.synchronize(device)
    return log_scalars.detach().clone(), last_loss_tensor, time.time() - training_start_time
...
def train_one_run(
    ...
    if should_use_cuda_graph(config, device):
        log_scalars, last_loss_tensor, training_elapsed_sec = replay_cuda_graph_training(
            hparams, dataset, prepared, config, device
        )
    else:
        ...
        + json.dumps(
            {
                ...
                "adam_eps": ADAM_EPS,
                "adam2_nesterov_options": ADAM2_NESTEROV_OPTIONS,
                "adam2_disable_bias1_options": ADAM2_DISABLE_BIAS1_OPTIONS,
                "adam2_adaptive_norm_options": ADAM2_ADAPTIVE_NORM_OPTIONS,
                "h_norms": H_NORMS,
                "wd": 0.0,
