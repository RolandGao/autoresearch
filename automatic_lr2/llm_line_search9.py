...
class MuonAdamW(torch.optim.Optimizer):
    ...
    @torch.no_grad()
    def step(self, collect_update_norms=False, only_kind=None):
        for group in self.param_groups:
            if only_kind is not None and group["kind"] != only_kind:
                continue
            if group["kind"] == "adamw":
...
INITIAL_GROUND_TRUTH_MATRIX_LR = 0.06
...
def compute_line_search_losses(model, batches):
    ...
    for inputs, targets in batches:
        with autocast_ctx:
            batch_losses = model(inputs, targets, reduction="none")
        losses.append(batch_losses.detach().flatten())
...
while True:
    ...
    line_search_results = evaluate_matrix_lr_grid(
        ...
        [optimizer],
        optimizer,
        line_search_validation_sets(line_search_batches),
