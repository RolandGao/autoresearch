...
DEFAULT_LOG = Path(__file__).with_name("cifar_baseline2_eval2_exp11.log")
DEFAULT_OUTPUT_DIR = Path(__file__).with_name("cifar_baseline2_eval2_exp11_plots")

SCHEDULER_ORDER = {
    "constant_2": 0,
    "last2_linear_2_to_0.1": 1,
}
...
def run_label(run):
    labels = {
        "constant_2": "best_lr const x2",
        "last2_linear_2_to_0.1": "best_lr x2 last2->0.1",
    }
...
def write_summary(runs, output_path):
    ...
    text = text.replace(
        ...
        "CIFAR Baseline Eval2 Exp11 Results",
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Plot cifar_baseline2_eval2 exp11 best-lr scheduler results."
    )
