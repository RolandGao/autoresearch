...
DEFAULT_LOG = Path(__file__).with_name("cifar_baseline2_eval5_exp2.log")
DEFAULT_OUTPUT_DIR = Path(__file__).with_name("cifar_baseline2_eval5_exp2_plots")
...
def plot_best_by_train_steps(runs, completions, output_path):
    ...
    steps = [run.train_steps for run in rows]
    fig, ax1 = plt.subplots(figsize=(8.2, 4.8))
    ax1.plot(steps, [run.train_loss for run in rows], marker="o", color="tab:blue", label="train loss")
    ax1.plot(steps, [run.val_loss for run in rows], marker="s", color="tab:orange", label="val loss")
    ax1.set_xlabel("train steps")
    ax1.set_ylabel("loss")
    ax1.grid(True, alpha=0.25)
    ax2 = ax1.twinx()
    ax2.plot(steps, [run.val_acc for run in rows], marker="^", color="tab:green", label="val acc")
    ax2.plot(steps, [run.tta_val_acc for run in rows], marker="D", color="tab:red", label="TTA val acc")
    ax2.set_ylabel("accuracy")
    lines = ax1.get_lines() + ax2.get_lines()
    ax1.legend(lines, [line.get_label() for line in lines], loc="center right", fontsize=8)
    fig.suptitle("Selected Schedule Metrics by Train Steps")
    fig.tight_layout()
    fig.savefig(output_path, dpi=180)
    plt.close(fig)
...
def write_summary(runs, search_steps, completions, cache_hits, output_path):
    lines = [
        "CIFAR Baseline Eval5 Exp2 Applied LR Schedule Search",
        "=" * 58,
        "",
...
def main():
    parser = argparse.ArgumentParser(
        description="Plot cifar_baseline2_eval5_exp2 applied LR schedule search."
    )
